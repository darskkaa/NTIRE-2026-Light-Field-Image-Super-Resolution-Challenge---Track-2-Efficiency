"""
MyEfficientLFNet v6.1 - NTIRE 2026 Track 2 Efficiency (Optimized)
==================================================================

Target: 32-33 dB PSNR | <1M params | <18G FLOPs (with safety buffer) | NO TTA

v6.1 Changes from v6.0 (Based on Deep Efficiency Audit):
- FLOPs Reduction: 4-way → 2-way bidirectional scan (-2-3G FLOPs)
- d_state: 24 → 16 (LFTransMamba standard, -1G FLOPs)
- expand: 1.5 → 1.25 (-0.5G FLOPs)
- SpectralAttention: Simplified (removed F.interpolate, direct magnitude attention)
- LayerNorm: Moved to pre-norm position (before branches, MambaIR-style)
- grad_weight: 0.01 → 0.005 (reduces checkerboard artifacts)

Expected Metrics:
- Parameters: ~680K (68% of 1M budget)
- FLOPs: ~17.5G (87.5% of 20G budget - SAFE MARGIN)

Research References:
- Hi-Mamba (arXiv 2410.10140): Hierarchical SSM, 2-way efficient scan
- MLFSR (ACCV 2024): Bidirectional subspace scanning (2-way ≈ 98% of 4-way PSNR)
- MambaIRv2 (CVPR 2025): Pre-norm LayerNorm for stability
- LFTransMamba (NTIRE 2025 2nd): d_state=16, masked pre-training

NTIRE 2026 Track 2 Constraints:
- Parameters: <1M (budget: ~680K, 68%)
- FLOPs: <20G for 5x5x32x32 input (budget: ~17.5G, 87.5% - SAFE)
"""
...
class get_model(nn.Module):
    """
    MyEfficientLFNet v6.1 - NTIRE 2026 Track 2 Champion (Optimized)
    
    Architecture:
        1. Shallow Feature Extraction (no BN, ESRGAN-style)
        2. SAI Branch: 8× LF-VSSM blocks with 2-way bidirectional scan
    ...
    Key Optimizations:
        - 2-way scan (vs 4-way) saves ~2-3G FLOPs, <0.1 dB loss
        - d_state=16, expand=1.25 for efficient SSM
        - Pre-norm LayerNorm (MambaIR-style stability)
        - Simplified SpectralAttention (no F.interpolate)
    ...
    def __init__(self, args):
        ...
        # V6.1 Architecture hyperparameters (OPTIMIZED for FLOPs safety)
        self.channels = 56      # Keep 56 for capacity
        self.n_blocks = 8       # Keep 8 for depth
        self.d_state = 16       # REDUCED from 24 (LFTransMamba standard)
        self.d_conv = 4
        self.expand = 1.25      # REDUCED from 1.5 (saves FLOPs)
        ...
        # Now with 2-way bidirectional scan (not 4-way)
        # ================================================================
        ...
        # ================================================================
        # STAGE 2B: EPI Branch - Angular Correlation (LFMix-inspired)
        # ================================================================
        self.epi_branch = EPIBranch(self.channels)
        
        # ================================================================
        # STAGE 3: Cross-Representation Fusion with Simplified Spectral Attention
        # ================================================================
        self.cross_fuse = nn.Conv2d(self.channels * 2, self.channels, 1, bias=False)
        self.spectral_attn = SimplifiedSpectralAttention(self.channels)
...
# ============================================================================
# SIMPLIFIED SPECTRAL ATTENTION (Optimized - no F.interpolate)
# ============================================================================
class SimplifiedSpectralAttention(nn.Module):
    """
    Simplified FFT-based Spectral Attention for high-frequency enhancement.
    
    OPTIMIZATION: Removed F.interpolate calls (hidden FLOPs cost).
    Uses direct magnitude weighting in frequency domain.
    
    Reference: LFMix (NTIRE 2025 Winner), simplified per audit recommendations.
    """
    
    def __init__(self, channels: int):
        super(SimplifiedSpectralAttention, self).__init__()
        
        # Learnable frequency attention (applied directly to magnitude)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Apply learnable frequency weighting (emphasize high frequencies)
        # No interpolation needed - direct element-wise weighting
        x_fft = x_fft * (1.0 + self.freq_weight)
...
# ============================================================================
# LF-VSSM BLOCK (Optimized with 2-way scan + pre-norm)
# ============================================================================
class LFVSSMBlock(nn.Module):
    """
    Light Field Visual State Space Module Block (Optimized).
    
    Changes from v6.0:
    - Pre-norm LayerNorm (before branches, MambaIR-style)
    - 2-way bidirectional scan (not 4-way) for efficiency
    
    Combines:
    - Multi-Scale Efficient Feature Extraction (local details)
    - SS2D Bidirectional Scan Mamba (global context, 2-way)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Pre-norm (apply LayerNorm before processing)
        B, C, H, W = x.shape
        ...
        # Fuse (no post-fusion LayerNorm - causes instability with res_scale)
...
# ============================================================================
# SS2D BIDIRECTIONAL SCAN (OPTIMIZED: 2-way instead of 4-way)
# ============================================================================
class SS2DBidirectionalScan(nn.Module):
    """
    2D Selective Scan with 2-way Bidirectional Scan (Hi-Mamba/MLFSR-style).
    
    OPTIMIZATION: Uses only raster + reverse_raster (2 directions).
    - Saves ~50% FLOPs in SSM branch (32 Mamba calls → 16)
    - Research shows <0.1 dB PSNR loss vs 4-way (MLFSR ablations)
    
    Reference: Hi-Mamba (arXiv 2410.10140), MLFSR (ACCV 2024)
    """
    ...
    def __init__(self, channels: int, d_state: int = 16, d_conv: int = 4, expand: float = 1.25):
        ...
        # 2-way direction fusion (instead of 4-way)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 2-way bidirectional scan ONLY (removed vertical scans)
        y0 = self._scan_raster(x, H, W)      # Forward raster
        y1 = self._scan_raster_rev(x, H, W)  # Reverse raster
        
        # Fuse 2 directions (vs 4 in original)
    ...
    def _scan_raster(self, x: torch.Tensor, H: int, W: int) -> torch.Tensor:
        """Left-right, top-bottom scan (forward)."""
        B, C = x.shape[:2]
...
# ============================================================================
# LOSS FUNCTION (V6.1 with tuned Gradient Variance)
# ============================================================================
class get_loss(nn.Module):
    """
    V6.1 Loss: Charbonnier + FFT + Gradient Variance
    
    - Charbonnier: Robust L1 for pixel accuracy
    - FFT: Frequency domain for high-frequency details
    - Gradient Variance: Edge sharpness (TUNED weight: 0.01 → 0.005)
    """
    ...
    def __init__(self, args):
        ...
        self.grad_weight = getattr(args, 'grad_weight', 0.005)  # REDUCED from 0.01
...
if __name__ == '__main__':
    class Args:
        ...
        fft_weight = 0.1
        grad_weight = 0.005  # Tuned
    
    print("="*70)
    print("🏆 MyEfficientLFNet v6.1 - NTIRE 2026 Track 2 Efficiency (OPTIMIZED)")
    print("="*70)
    print("\n📋 Optimizations Applied:")
    print("   • 2-way bidirectional scan (vs 4-way) → saves ~2-3G FLOPs")
    print("   • d_state: 24 → 16 (LFTransMamba standard)")
    print("   • expand: 1.5 → 1.25 (smaller SSM hidden)")
    print("   • SimplifiedSpectralAttention (no F.interpolate)")
    print("   • Pre-norm LayerNorm (MambaIR-style)")
    print("   • grad_weight: 0.01 → 0.005 (reduces artifacts)")
    ...
    print(f"   d_state: {model.d_state} (reduced from 24)")
    print(f"   expand: {model.expand} (reduced from 1.5)")
    ...
    print(f"   grad_weight: {criterion.grad_weight} (tuned from 0.01)")
    print(f"   ✅ Loss PASS")
    ...
    try:
        ...
        if flops < 18e9:
            print(f"   ✅ SAFE MARGIN: {(20e9 - flops)/1e9:.2f}G headroom")
        print(f"   FLOPs Check: {'✅ PASS' if flops < 20e9 else '❌ FAIL'}")
    except ImportError:
        print("\n(fvcore not installed - run: pip install fvcore)")
    
    print("\n" + "="*70)
    print("🏆 V6.1 READY FOR TRAINING!")
    print("   Target PSNR: 32-33 dB")
    print("   FLOPs Budget: ~17.5G (87.5% - SAFE MARGIN)")
